...
%     for j=1:size(A,2)
%         thgreen=10;
%         if A(i,j,1)<91+thgreen && 91-thgreen<A(i,j,1) && ...
%            A(i,j,2)<179+thgreen && 179-thgreen<A(i,j,2) && ...
%            A(i,j,3)<120+thgreen && 120-thgreen<A(i,j,3)
%             img(i,j)=0;
%         end
%     end
% end
...
% imshow(Imp)
imshow(C,[])
title('Informed RRT* Search path Map');
xL=size(Imp,1);
yL=size(Imp,2);
hold on
...
for iter = 1:10000
    ...
    for k = 1:M
        ...
        if T.v(nearptr(k,1)).dist >  (T.v(count).dist + norm(x_1-x_new))
            ...
            hold on;
            plot([x_1(1),x_new(1)],[x_1(2),x_new(2)],'-r');
            hold on;
        end
    end
...
% for i = 1:size(A,1)
%     for j = 1:size(A,2)
%         if isKey(myMap, num2str(i*10000+j))
%             A(i, j) = 0;
%         end
%     end
% end
...
title('Informed RRT* path Map');
% legend('RRT*')
legend('start','goal','IRRT* path');
toc
